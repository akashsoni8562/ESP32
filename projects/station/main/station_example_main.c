    ...
    switch (evt->event_id) {
        case HTTP_EVENT_ON_DATA: {
            if (!esp_http_client_is_chunked_response(evt->client)) {
                size_t available = sizeof(http_response) - http_response_len - 1;
...
static void http_get_weather(void)
{
    ...
    ESP_LOGI(TAG, "Checking weather via HTTP...");
    esp_err_t err = esp_http_client_perform(client);
    if (err == ESP_OK) {
        int status = esp_http_client_get_status_code(client);
        ESP_LOGI(TAG, "HTTP status = %d", status);
        if (status == 200) {
            ESP_LOGI(TAG, "Weather: %s", http_response);
        } else {
...
void app_main(void)
{
    ...
    if (wifi_init_sta()) {
        if (check_internet_connectivity()) {
            ESP_LOGI(TAG, "Internet available, fetching weather...");
            http_get_weather();
        } else {
            ESP_LOGW(TAG, "WiFi is connected but internet is unavailable");
        }
    }
}
